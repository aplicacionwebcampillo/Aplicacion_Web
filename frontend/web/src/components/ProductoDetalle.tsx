...
export default function ProductoDetalle() {
  ...
  useEffect(() => {
    fetch(`https://aplicacion-web-m5oa.onrender.com/productos/${nombre}`)
      ...
      .then((data) => {
        setProducto(data);
        const tallas = extraerTallas(data.descripcion);
        setTallasDisponibles(tallas);
        setTallaSeleccionada(tallas[0] || ""); // por defecto la primera
      })
      .catch((err) => console.error("Error al cargar producto:", err));
  }, [nombre]);

  const extraerTallas = (descripcion: string): string[] => {
    const match = descripcion.match(/Tallas:\s*(.*)/i);
    if (!match) return [];

    const tallasRaw = match[1].trim();

    // Caso especial: talla única o una sola talla
    if (/^única$/i.test(tallasRaw) || !tallasRaw.includes(",")) {
      return [tallasRaw];
    }
          ...
          {tallasDisponibles.length > 0 && (
            <div className="mb-4">
              ...
              <select
                ...
                className="rounded-[1rem] font-poetsen border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
